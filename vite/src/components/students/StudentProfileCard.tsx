...
const StudentProfileCard = ({ student }:any) => {
  return (
    <div className="max-w-xs rounded-lg overflow-hidden shadow-lg bg-white p-2 m-2 hover:shadow-2xl transition-shadow duration-300 ease-in-out">
      <img className="w-20 h-20 rounded-full mx-auto border-2 border-blue-300 shadow-md mt-2" src={student.imageUrl} alt={student.name} />
      <div className="text-center mt-2">
        ...
        <Link to={`/profile/${student.id}`} className="mt-2 inline-block bg-[#395241] text-white font-semibold py-2 px-2 rounded-2xl transition-colors duration-300">
          View more
        </Link>
      </div>
    </div>
